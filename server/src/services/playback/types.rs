...
pub struct ResyncState {
    pub id: Snowflake,
    timestamps: Mutex<HashMap<Snowflake, Option<u32>>>,
}
...
impl ResyncState {
    ...
    pub async fn add_timestamp(&self, user: Snowflake, timestamp: u32) -> bool {
        let mut timestamps = self.timestamps.lock().await;
        timestamps.entry(user).insert_entry(Some(timestamp));
        timestamps.values().all(|t| t.is_some())
    }

    pub async fn get_min_timestamp(&self) -> Option<u32> {
        let timestamps = self.timestamps.lock().await;
        timestamps.values().filter_map(|t| *t).min()
    }
}
